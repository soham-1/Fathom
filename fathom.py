    ...
    def __init__(self, master):
        ...
        if result is not None:
            for (message,i) in zip(result, range(len(result)) ):
                Label(self, text=f"{message[1]} :   ", fg="green").grid(row=i, column=0)
                Label(self, text=f"{message[3]} ").grid(row=i, column=1)
        st = ScrolledText(self, width=50, height=2)
        st.grid(row=1000, column=0)
        Button(self, text="send", command= lambda : self.send(user, reciever, st.get(0.0, END))).grid(row=1000, column=1)


    def send(self, user, reciever, msg):
        if len(msg)==1 :
            pass
        else:
            mycursor.execute(" insert into chatbox(sender, reciever, message) values(%s,%s,%s)",(user, reciever, msg))
            myconnector.commit()
